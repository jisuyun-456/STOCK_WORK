...
3. `_AutoResearch/STOCK/wiki/log.md` → 마지막 AutoResearch iteration 상태 확인 (Obsidian MCP 또는 직접 읽기)
실행 후 "현재 상태 요약 + AutoResearch 현황 + 다음 추천 태스크 1개" 말해줄 것.
세션 종료 시: git commit 필수.

## Graphify Knowledge Graph
코드베이스 knowledge graph (`pip install graphifyy` 기반, Karpathy Wiki 패턴):
- 그래프 위치: `graphify-out/graph.json` (768 nodes, 1673 edges, 41 communities)
